...
class DB_Client:
    ...
    def get_devices_locations(self):
        cursor = self.get_cursor()
        cursor.execute(f'select ST_Y(location) as latitude, ST_X(location) as longitude from {DB_SCHEMA}.view_devices_smartphone_location_gis_stricted dl;')
    ...
    def get_devices_smartphone_location_altitude_m(self):
        cursor = self.get_cursor()
